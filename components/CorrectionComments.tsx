  ...
  return (
    <>
      <button
        ...
        className="text-xs text-ink-muted hover:text-base-content flex items-center gap-1"
      >
      ...
      <SlideDrawer open={open} onClose={() => setOpen(false)} title={triggerLabel}>
        {loading ? (
          <p className="text-sm text-ink-faint text-center py-8">{dict.common.loading}</p>
        ) : (
          <CommentSection
            entryId={entryId}
