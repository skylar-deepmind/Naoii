  ...
  return (
    <AppShell>
      <div className="py-8 sm:py-12 max-w-3xl">
        ...
        <p className="text-ink-muted mb-10">{intro}</p>
        <div className="space-y-10">
          {versions.map((v, vi) => (
            <div key={vi}>
              <div className="flex items-center gap-3 mb-4">
                <Badge variant="primary" size="sm">{v.version}</Badge>
                <span className="text-sm text-ink-muted">{v.date}</span>
              </div>
              {v.intro && <p className="text-sm text-foreground/70 mb-4">{v.intro}</p>}
              {v.sections.map((s, si) => (
                <div key={si} className="mb-4">
                  {s.heading && <h3 className="text-sm font-semibold mb-2">{s.heading}</h3>}
                  <ul className="space-y-1.5">
                    {s.items.map((item, ii) => (
                      <li key={ii} className="text-sm text-foreground/70 flex items-start gap-2">
              ...
              {v.notes && <p className="text-sm text-ink-faint italic mt-4">{v.notes}</p>}
            </div>
          ))}
        </div>
      </div>
