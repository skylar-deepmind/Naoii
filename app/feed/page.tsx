...
export default async function FeedPage({ searchParams }: Props) {
  ...
  const { entries, nextCursor } = await getFeedEntries({ tab: currentTab, completeness, cursor });
      ...
      {entries.length === 0 ? (
        ...
        <>
          <div className="space-y-3">
            {entries.map((entry) => (
              <PostCard key={entry.id} {...entry} typeLabels={dict.typeLabels} completenessLabels={dict.completeness} correctionLabel={dict.correction.label} adoptedLabel={dict.post.accepted} timeLabels={dict.time} />
            ))}
          </div>
          {nextCursor && (
